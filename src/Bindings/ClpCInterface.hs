-- | Haskell bindings for the C interface of the linear programming solver CLP (v1.15.10).
module Bindings.ClpCInterface where


import Foreign.C.String (CString)
import Foreign.C.Types  (CDouble (..), CInt (..), CUChar (..))
import Foreign.Ptr      (Ptr, FunPtr)


newtype ClpSimplex = ClpSimplex (Ptr ())
newtype ClpSolve   = ClpSolve (Ptr ())
...
foreign import ccall "Clp_loadProblem"                   loadProblem :: ClpSimplex -> CInt -> CInt -> Ptr CInt -> Ptr CInt -> Ptr CDouble -> Ptr CDouble -> Ptr CDouble -> Ptr CDouble -> Ptr CDouble -> Ptr CDouble -> IO ()
